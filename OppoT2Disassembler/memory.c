...
// Public things
uint32_t readRAM(const uint32_t address) {
    uint32_t offset;
    const uint32_t* page = translate_address(address, &offset);
    return page[offset];
}

void writeRAM(const uint32_t address, const uint32_t data) {
    uint32_t offset;
...
// Helper Functions
// Return pointer to memory page and offset.
uint32_t* translate_address(const uint32_t address, uint32_t* offset) {
...
uint32_t page_to_address(const uint32_t page, const uint32_t offset) {
    return (page * PAGE_SIZE) + offset;
}

uint32_t address_to_page(const uint32_t address, uint32_t* offset) {
    *offset = address % PAGE_SIZE;
    return address / PAGE_SIZE;
}

void allocate_page(const uint32_t page_index) {
